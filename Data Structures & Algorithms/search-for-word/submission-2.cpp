...
public:
    bool ans = false;
    string word_;
    set<pair<int,int>> arr;  // renamed from vector<vector<int>>

    bool check_board(int i, int j) {
        return arr.count({i, j});
    }

    bool helper(vector<vector<char>>& board, int idx, int i, int j) {
        if (idx == (int)word_.size()) { return true; }

        if (i < 0 || j < 0 || i >= (int)board.size() || j >= (int)board[0].size() ||
            board[i][j] != word_[idx] || check_board(i, j)) {
            return false;
        }

        arr.insert({i, j});
        bool res = helper(board, idx+1, i+1, j) ||
                   helper(board, idx+1, i-1, j) ||
                   helper(board, idx+1, i, j+1) ||
                   helper(board, idx+1, i, j-1);
        arr.erase({i, j});
        return res;
    }
